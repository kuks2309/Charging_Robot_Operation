        ...
        # Eye in Hand 탭 (인덱스 4)이 선택되면 Tool Frame 4로 설정
            ...
            if self.robot and self.robot.is_connected:
                try:
                    ...
                    self._log(f"Tool Frame 설정 오류: {e}")
        # 레이저 캘리브레이션 탭 (인덱스 6) → TF4 강제 + ArduCam 강제 전환
        elif index == 6:
            if self.robot and self.robot.is_connected:
                try:
                    success, msg = self.robot.send_set_toolframe(4, wait=True)
                    if success:
                        self._log("Laser Calibration 탭 선택: TF4 설정 완료")
                        self._update_statusbar()
                    else:
                        self._log(f"Laser Calibration TF4 설정 실패: {msg}")
                except Exception as e:
                    self._log(f"Laser Calibration TF4 설정 오류: {e}")
            self._on_camera_type_changed(CAMERA_ARDUCAM)
